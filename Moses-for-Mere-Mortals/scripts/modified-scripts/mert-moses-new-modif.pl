...
# This script was now slightly changed so as to create a new maxruns parameter that can limit the maximal number of tuning iterations
# This script is subject to the LGPL licence
# Usage:
...
Options:
  ...
  --prev-aggregate-nbestlist=INT  ... number of previous step to consider when loading data (default =-1)
                                      ...
                                      1 means 1 previous data , i.e. from the actual iteration and from the previous one
                                      and so on 
  --max-runs=INT ... Maximal number of runs 
